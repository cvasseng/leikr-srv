...
module.exports = (function () {
	...
	return {
		...
		*/
		setTile: function (tx, ty, tile, actor) {
			console.log('setting tile at ' + tx + ',' + ty);
			doForTileAt(tx, ty, function (chunk, index) {
				
				if (typeof actor !== 'undefined') {
					//Check if the removed tile is a minable.
					//If it is, give the resulting item to the actor.
				...
				emit('tile_change', chunk, tx, ty, chunk.data[index]);
			});
		},
		...
		*/
		...
		tick: function () {
			...
			_deltaTime = (timeMs - _deltaCounter) / 1000;

			_activeProjectiles = _activeProjectiles.filter( function (projectile) {
				var updated = projectile.update(timeMs, _deltaTime);
				if (!updated) {
					//Emit projectile_hit
