...
//Projectile object
var Projectile = function (origin, originator, direction, decay, speed, spritesheet, ondone) {
	...
	this.update = function (timeMs, deltaTime) {
		...
		if (typeof chunk !== 'undefined') {
			for (var i = 0; i < chunk.actors.length; i++) {
				...
				if (_originator !== null && _originator.id === actor.id) {

				} else if (actor.pointInActor(this.pos.x, this.pos.y)) {
					doDone('actor', actor);
					return false;
				}
			}
...
module.exports = {
	create: function (origin, originator, direction, decay, speed, spritesheet, ondone) {
		return new Projectile(origin, originator, direction, decay, speed, spritesheet, ondone);
	}
}
